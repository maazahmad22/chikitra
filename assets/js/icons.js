/* ==================================================================
   icons.js — Lucide icon set (embedded)
   ------------------------------------------------------------------
   Lucide icon path data inlined as SVG so the demo renders identically
   offline, with no icon flash and no CDN dependency.

   Usage:  icon('calendar-days', 'h-5 w-5')
           Icons.hydrate(root)  ->  replaces <i data-icon="name"></i>
   ================================================================== */
(function (global) {
  'use strict';
  ...
  const P = {
    ...
    'sparkles': '<path d="m12 3-1.9 5.8a2 2 0 0 1-1.287 1.288L3 12l5.8 1.9a2 2 0 0 1 1.288 1.287L12 21l1.9-5.8a2 2 0 0 1 1.287-1.288L21 12l-5.8-1.9a2 2 0 0 1-1.288-1.287Z"/><path d="M5 3v4"/><path d="M3 5h4"/>',
  ...
  function icon(name, cls) {
    const body = P[name];
    if (!body) return '';
    return '<svg ' + ATTRS + ' class="' + (cls || 'h-5 w-5') + '" aria-hidden="true" focusable="false">' + body + '</svg>';
  }
  ...
  const Icons = {
    ...
    has: (n) => !!P[n],
    names: () => Object.keys(P),
    /** Replace <i data-icon="name" data-class="..."></i> placeholders in a subtree. */
    hydrate(root) {
      (root || document).querySelectorAll('i[data-icon]').forEach((el) => {
        const svg = icon(el.dataset.icon, el.dataset.class || 'h-5 w-5');
